...
def play_turn(player_x, player_o, player_name, token, board1_squares)
    ...
    while true
        ...
        if GameValidator.new.is_valid(move) && GameValidator.new.is_unique(move, board1_squares)
            # puts "move is valid and unique"
            add_move(token, move, board1_squares)
            # puts "added move"
            GameOutput.new.print_game(player_x, player_o, board1_squares, x_tally, o_tally)
            break
        ...
        elsif !GameValidator.new.is_unique(move, board1_squares)
